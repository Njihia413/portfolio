...
export default function LayoutWrapper({ children }: LayoutWrapperProps) {
  ...
  useEffect(() => {
    ...
    const servicesWidget = document.querySelector(".services-widget") as HTMLElement | null;
    if (servicesWidget) {
      const activeBg = servicesWidget.querySelector(".active-bg") as HTMLElement | null;

      function updateActiveService(element: HTMLElement | null) {
        if (!element || !servicesWidget) return;
      ...
      function handleMouseLeave() {
        const currentElement = servicesWidget?.querySelector(".current") as HTMLElement | null;
      ...
      // Initial call
      updateActiveService(servicesWidget.querySelector(".current") as HTMLElement | null);
    ...
    if (animates.length > 0) {
      animates.forEach((animate) => {
        animate.addEventListener("animationend", (e) => {
          setTimeout(() => {
            (e.target as HTMLElement).setAttribute("animation-end", "");
          }, 1000);
        });
      });
    }
  ...
  useEffect(() => {
    const animatedTextElements = document.querySelectorAll<HTMLElement & { animation?: gsap.core.Tween; split?: SplitText }>(
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            const element = entry.target as HTMLElement & { animation?: gsap.core.Tween; split?: SplitText };
